...
import 'package:sartor_order_management/features/orders/widgets/service_grid_card.dart';
...
void main() {
  ...
  group('Complete App Flow', () {
    ...
    final serviceToEdit = Service(id: 1, name: 'Old Service Name', price: 100.0, category: ServiceCategory.mensWear);

    when(mockServiceRepository.getServices()).thenAnswer((_) async => [serviceToEdit]);
    when(mockServiceRepository.updateService(any<Service>())).thenAnswer((_) async {});
    ...
    when(mockOrderRepository.getUserOrders(any<String>())).thenAnswer((_) async => []);
    ...
    testWidgets('should complete the full user flow from service editing to order verification', (WidgetTester tester) async {
      ...
      await tester.tap(find.byType(ServiceGridCard).first);
      await tester.pumpAndSettle();
